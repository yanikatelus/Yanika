                ...
                <div className="details">
                  ...
                  <div className="navigation-buttons">
                    {experiences.map((experience) => (
                      <button
                        key={experience.id}
                        onClick={() => handleExperienceChange(experience.id )}  // Adjusted for zero-based index if needed
                        className={`nav-button  ${experience.id  === activeIndex ? 'active' : ''}`}
                      >
                        {experience.role.includes(',') ? experience.role.split(', ')[1] : experience.role}
                      </button>
                    ))}
                  </div>
                </div>
